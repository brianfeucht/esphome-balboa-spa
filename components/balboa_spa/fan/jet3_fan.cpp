...
static const char *TAG = "balboa_spa.jet3_fan";

namespace esphome {
namespace balboa_spa {

void Jet3Fan::update(SpaState* spaState) {
    bool new_state = spaState->jet3 > 0;
    int new_speed = spa_state_to_speed(spaState->jet3);
...
void Jet3Fan::set_parent(BalboaSpa *parent) {
    spa = parent;
...
void Jet3Fan::control(const fan::FanCall &call) {
    if (call.get_state().has_value()) {
        if (*call.get_state()) {
            ...
            spa->set_jet3_state(target_state);
        } else {
            spa->set_jet3_state(0);
        }
...
int Jet3Fan::spa_state_to_speed(uint8_t spa_state) {
    switch (spa_state) {
        case 0: return 0;  // Off
        case 1: return 1;  // Low
...
uint8_t Jet3Fan::speed_to_spa_state(int speed) {
